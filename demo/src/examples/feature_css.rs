...
pub fn Example(debug: Signal<bool>, data: Signal<Vec<MyData>>) -> impl IntoView {
    let series = Series::new(|data: &MyData| data.x).line(
        Line::new(|data: &MyData| data.y1)
            .with_name("sunlight")
            // The default colors assume a light background but this is easily
    ...
    view! {
        ...
        <Style>"
            .my-theme {
                ...
                /* Use 'fill' for filling text color */
                fill: #c0c5ce;
        ...
        <div class="my-theme">
            <Chart
                ...
                inner=[
                    ...
                    // We can also use the `with_color` method on some elements
                    YGridLine::default().with_color("#505050".parse::<Color>().unwrap()).into_inner(),
...
fn my_scheme() -> ColorScheme {
    STACK_COLOR_SCHEME.into()
}
